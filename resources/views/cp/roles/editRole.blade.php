                        ...
                        <div class="card-body table-responsive">
                            <form action="/admincp/roles" id="role" enctype="multipart/form-data" method="post">
                                ...
                                <div class="form-group">
                                    <label>Name</label>
                                    <input name="name" value="{{$role['name']}}" id="name" type="text" class="form-control" placeholder="Enter the name of this role." required="required">
                                </div>
                                        ...
                                        @foreach ($permissions as $permission => $name)
                                            <div class="form-group col-3">
                                                <label class="checkbox-inline">
                                                    @if (in_array($permission, json_decode($role['permissions'])))
                                                        <input name="permissions[]" type="checkbox" value="{{$permission}}" checked> {{$name}}
                                                    @else
            ...
            $.ajax({
                ...
                success:function(Messages){
                    ...
                    for (var key in Messages) {
                        if (Messages[key]== "The role was updated successfully."){
